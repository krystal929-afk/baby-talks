...
function CalendarPage() {
  ...
  const { upcoming, past } = useMemo(() => {
    ...
    events.forEach((event) => {
      if (new Date(event.starts_at).getTime() >= now - 3600_000) up.push(event);
      else pa.push(event);
    });
  ...
  return (
    <div className="bf-screen bf-calendar">
      <main className="bf-shell">
        <Link to="/" className="mb-2 inline-flex items-center gap-1 text-[9px] uppercase tracking-[.16em] text-[#8f8880]">
        ...
        <header className="bf-calendar-head">
          ...
          <div className="mt-1 text-center text-[11px] text-[#6f20b6]">xoxo Baby ♥</div>
        </header>
        ...
        <div className="bf-calendar-sheet mb-5">
          <MonthCalendar
            mode="single"
            ...
            modifiersClassNames={{
              hasEvent: "relative after:absolute after:bottom-1 after:left-1/2 after:h-1 after:w-1 after:-translate-x-1/2 after:rounded-full after:bg-[#9d3ee7]",
            }}
            classNames={{
              month_caption: "mb-2 flex h-9 w-full items-center justify-center px-8",
              caption_label: "bf-paper-title !px-6 !py-2 !text-[13px]",
            }}
            className="mx-auto"
          />
        </div>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-16 text-[#8f8880]"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Diggin&apos; through the date book…</div>
        ) : events.length === 0 ? (
          <div className="bf-card p-7 text-center"><h3 className="text-lg">Nothin&apos; on the books, daddy</h3><p className="mt-2 text-xs text-[#8f8880]">Tell Baby in chat or tap Add event.</p></div>
        ) : (
          <>
...
function Section({ title, tagline, children }: { title: string; tagline: string; children: React.ReactNode }) {
  return (
    <section className="mb-6">
      <div className="mb-3 flex items-baseline gap-2 border-b border-[#6f20b6]/40 pb-2">
        <h2 className="bf-calendar-section-title">{title}</h2>
        <span className="text-[10px] text-[#7d756e]">{tagline}</span>
      </div>
...
function EventCard({ event, onChanged, faded }: { event: Event; onChanged: () => void; faded?: boolean }) {
  ...
  const weekday = starts.toLocaleDateString(undefined, { weekday: "long" }).toUpperCase();

  return (
    <div className={cn("bf-event-card", faded && "opacity-55")}>
      <div className="bf-event-layout">
        <div className="bf-event-date" aria-hidden>
          <span className="bf-event-weekday">{weekday}</span>
          <span className="bf-event-month">{month}</span>
          <strong>{day}</strong>
          <span className="bf-event-skull">☠</span>
        </div>

        <div className="min-w-0 flex-1">
          <div className="mb-1 text-[9px] uppercase tracking-[.16em] text-[#9d3ee7]">{event.all_day ? "All day" : "Filed event"}</div>
          <h3>{event.title}</h3>
          <div className="mt-2 space-y-1 text-[10px] text-[#8f8880]">
            <span className="flex items-start gap-1.5"><Clock className="mt-0.5 h-3 w-3 shrink-0 text-[#9d3ee7]" />{fmt(event.starts_at, event.all_day)}</span>
            {event.location && <span className="flex items-start gap-1.5"><MapPin className="mt-0.5 h-3 w-3 shrink-0 text-[#9d3ee7]" />{event.location}</span>}
            {event.remind_at && <span className="flex items-start gap-1.5"><Bell className="mt-0.5 h-3 w-3 shrink-0 text-[#9d3ee7]" />{fmt(event.remind_at, false)}</span>}
          </div>
          {event.notes && <p className="mt-3 text-xs leading-relaxed text-[#b5aca1]">{event.notes}</p>}
        </div>

        <button
          onClick={() => { if (confirm("Burn this one, daddy?")) del.mutate(); }}
          className="self-start p-1 text-[#756f68] hover:text-destructive"
          aria-label="Delete"
        >
...
function AddEventDialog({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 sm:items-center sm:p-4" onClick={onClose}>
      <div className="w-full max-w-md border border-[#6f20b6]/50 bg-[#09070b] p-5 sm:rounded-sm" onClick={(event) => event.stopPropagation()}>
        <h2 className="bf-paper-title !mx-0 mb-5 text-base">New event</h2>
        <div className="space-y-3">
          <Field label="Title"><Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Studio session" /></Field>
          <Field label="When"><Input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></Field>
          <Field label="Location"><Input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Optional" /></Field>
          <Field label="Notes"><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={3} placeholder="Optional" /></Field>
        </div>
        <div className="mt-5 flex justify-end gap-2">
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button onClick={save} disabled={!title.trim() || saving} className="bf-btn-primary">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save"}</Button>
        </div>
      </div>
    </div>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><label className="bf-kicker mb-1 block text-[9px]">{label}</label>{children}</div>;
}
